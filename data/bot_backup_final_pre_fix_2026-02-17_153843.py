...
# --- Configuration ---
TOKEN = "8228071414:AAG31gr_raDybAdi_kNkyGZ8mpzBkiZX0VU" # Your Telegram Bot API Token
...
def migrate_json_to_db():
    ...
    if os.path.exists(users_json_file):
        try:
            ...
            os.remove(users_json_file) # Remove after successful migration
    ...
    if os.path.exists(trips_json_file):
        try:
            with open(trips_json_file, 'r', encoding='utf-8') as f:
    ...
    if os.path.exists(drafts_json_file):
        try:
            ...
            os.remove(drafts_json_file) # Remove after successful migration
            print(f"Migrated {len(old_drafts)} drafts from JSON to DB.")
        except Exception as e:
            print(f"Error migrating drafts.json: {e}")
...
def get_my_stats(trip_id, my_uid):
    ...
    conn.close() # Close connection after fetching user_names
...
def send_all_expenses_list(chat_id, user_id, message_id=None, page=0):
    ...
    if not trip: # Trip might have been deleted or corrupted
        ...
        user['joined_trips'] = [t for t in user['joined_trips'] if t != tid] # Remove invalid trip from joined_trips
        db_save_user(user_id, user)
        return send_message(chat_id, "Активная поездка не найдена. Выберите другую или создайте новую.", reply_markup={"inline_keyboard": [[{"text": "🗂 Архив обязательств", "callback_data": "MENU_TRIPS"}]]})
    ...
    if not expenses:
        ...
        keyboard = {"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]}
    ...
    keyboard_rows.append([{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}])
...
def send_help_menu(chat_id):
    msg = (\
        "📖 *Инструкция (Splitopus v1.0 Audit Edition):*\\n\\n"\
        "💸 **Финансовый аудит:**\\n"\
        "• `500 Обед` — Зафиксировать расход\\n"\
        "• `/balance` — Отчет по балансу и обязательствам\\n"\
        "• **[💸 Вернуть долг]** — Зафиксировать операцию возврата средств\\n"\
        "• `/me` — Ваша персональная финансовая сводка\\n\\n"\
        "📝 **Заметки для аудита:**\\n"\
        "• `/note Код 1234` — Сохранить заметку к делу\\n"\
        "• `/notes` — Просмотреть все заметки\\n\\n"\
        "🎲 **Развлечения (не для аудита):**\\n"\
        "• `/roulette` — Кто принимает удар на себя?\\n\\n"\
        "⚙️ **Управление кейсами:**\\n"\
        "• `/trips` — Переключить/архивировать финансовые кейсы\\n"\
        "• `/setrate` — Обновить курс валют\\n"\
        "• `/export` — Экспортировать отчет по кейсу в CSV"\
    )
...
def notify_others(tid, payer_id, amount, desc, category, split_map):
    ...
    markup = {"inline_keyboard": [[{"text": "📊 Мой Баланс", "callback_data": "SHOW_MY_BALANCE"}]]} # SHOW_MY_BALANCE is now handled by MENU_BALANCE
...
def handle_command(chat_id, user_id, user_name, text):
    ...
    if cmd == "/start":
        keyboard = {"inline_keyboard": [\
            ...
            [{"text": "Подключиться к чужим долгам", "callback_data": "MENU_JOIN"}],\
            [{"text": "Архив обязательств", "callback_data": "MENU_TRIPS"}]\
        ]}\
        ...
        if tid:
            ...
            if trip:
                ...
                keyboard["inline_keyboard"].insert(0, [{"text": f"Оперативный штаб поездки ({t_name})", "callback_data": "OPEN_DASHBOARD"}])
                status = f"Активная поездка: *{t_name}*\\nБаланс ещё можно спасти.\\n" # Updated status message
            
        msg = (\
            "🐙 *Splitopus v1.0 (Audit Edition)*\\n\\n"\
            "Финансовые ошибки вашей компании уже зафиксированы.\\n"\
            f"{status}"\
        )
        send_message(chat_id, msg, reply_markup=keyboard)\
        ...
        tid = user.get('active_trip_id')
        if not tid: return send_message(chat_id, "Нет активной поездки.", reply_markup={"inline_keyboard": [[{"text": "🔙 В главное меню", "callback_data": "BACK_MAIN"}]]})
        
        note_text = " ".join(args)
        if not note_text: return send_message(chat_id, "Пример: `/note Код 1234`", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        db_save_trip(trip)
        send_message(chat_id, "✅ Заметка сохранена!", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        if not notes: return send_message(chat_id, "📝 Заметок пока нет.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        send_message(chat_id, msg, reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        if not members: return send_message(chat_id, "В поездке нет участников для рулетки.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        send_message(chat_id, f"🎯 Сегодня принимает удар: *{victim_name.upper()}*! 🎉", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        send_message(chat_id, "✅ Отчет экспортирован.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        try:
            ...
            send_message(chat_id, f"✅ Курс установлен: 1 {curr} = {rate} RUB.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        except (ValueError, IndexError):
            send_message(chat_id, "❌ Пример: `/setrate 2.8`", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
...
def handle_text(chat_id, user_id, user_name, text):
    ...
    if state == "WAITING_ROULETTE_AMOUNT":
        try:
            ...
            if not tid or not payer_id:
                send_message(chat_id, "⚠️ Произошла ошибка с рулеткой. Попробуйте снова.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
                user['state'] = "IDLE"
                db_save_user(user_id, user)
                return
            ...
            members = trip['members']
            if not members:
                send_message(chat_id, "В поездке нет участников для рулетки.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
                user['state'] = "IDLE"
                user['roulette_trip_id'] = None
                user['roulette_payer_id'] = None
                db_save_user(user_id, user)
            ...
            send_message(chat_id, f"✅ Расход по рулетке *{amount:,.0f}* добавлен!", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            notify_others(tid, payer_id, amount, "Расход по рулетке", "FUN", split_map)

        except ValueError:
            send_message(chat_id, "❌ Введите числовое значение суммы.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if state == "WAITING_REPAYMENT_AMOUNT":
        try:
            ...
            if not tid or not target_uid:
                send_message(chat_id, "⚠️ Произошла ошибка с возвратом долга. Попробуйте снова.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
                user['state'] = "IDLE"
            ...
            send_message(chat_id, f"✅ Вы вернули *{amount:,.0f}* пользователю *{target_name}*.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            send_message(target_uid, f"💸 *{user_name}* вернул вам долг: *{amount:,.0f}*")
            
        except ValueError: send_message(chat_id, "❌ Введите число.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if state == "WAITING_TRIP_CODE":
        ...
        if found_tid:
            ...
            send_message(chat_id, f"✅ Вы присоединились! Активная поездка: `{trip.get('name')}`", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            time.sleep(1)
            send_trip_dashboard(chat_id, user_id) 
        else:
            send_message(chat_id, "❌ Неверный код.", reply_markup={"inline_keyboard": [[{"text": "🔙 В главное меню", "callback_data": "BACK_MAIN"}]]})
        return
        ...
        try:
            ...
            if tid:
                ...
                if trip:
                    ...
                    code = trip['code']
                    curr = trip['currency']
                    send_message(chat_id, f"✅ Поездка создана!\\nВалюта: *{curr}* (Курс: {rate})\\n🔑 Код: `{code}`", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
                    time.sleep(1)
                    send_trip_dashboard(chat_id, user_id)
            user['state'] = "IDLE"
            db_save_user(user_id, user)
        except ValueError:
            send_message(chat_id, "❌ Введите число (например 2.8):", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if state == "WAITING_CUSTOM_CURRENCY":
        ...
        db_save_user(user_id, user)
        send_message(chat_id, f"💱 Какой курс 1 {curr} к рублю? (например: `2.8`):", reply_markup={"inline_keyboard": [[{"text": "❌ Отмена", "callback_data": "OPEN_DASHBOARD"}]]}) # Added cancel button
        return
    ...
    if state == "WAITING_CUSTOM_SPLIT":
        try:
            ...
            if not draft: 
                send_message(chat_id, "⚠️ Время вышло. Начните ввод расхода заново.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
                user['state'] = "IDLE"
                user['draft_id'] = None
                db_save_user(user_id, user)
                return
            ...
            members = trip['members']
            if len(amounts) != len(members): return send_message(chat_id, f"❌ Нужно {len(members)} сумм, введено {len(amounts)}. Введите суммы через пробел.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            if abs(sum(amounts) - draft['amount']) > 0.01: # Allow small floating point deviation
                return send_message(chat_id, f"❌ Сумма не сходится. Ожидалось: {draft['amount']:,.0f}, получено: {sum(amounts):,.0f}. Повторите ввод.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            ...
            send_message(chat_id, f"✅ Сохранено: *{draft['amount']:,.0f}* ({CATEGORIES.get(draft.get('category'), draft.get('category'))})\\n", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            notify_others(draft['trip_id'], draft['payer_id'], amount, draft['description'], draft.get('category'), split_map)
                    
        except ValueError: send_message(chat_id, "❌ Введите числа. Пример: `100 200 300`", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if state == "IDLE": 
        try:
            ...
            if not tid: return send_message(chat_id, "Сначала создайте или вступите в поездку!", reply_markup={"inline_keyboard": [[{"text": "🔙 В главное меню", "callback_data": "BACK_MAIN"}]]})
    ...
    # Fallback for unexpected states, if text is received and not consumed by any state handler
        ...
        send_message(chat_id, "Не понял ваш ввод. Пожалуйста, следуйте инструкциям или вернитесь в меню.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
...
def send_category_menu(chat_id, draft_id, curr, message_id=None):
    ...
    if not draft:
        send_message(chat_id, "⚠️ Время для выбора категории вышло. Начните ввод расхода заново.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        # Reset user state
...
def send_split_menu(chat_id, user_id, draft_id, message_id=None):
    draft = db_get_draft(draft_id)
    if not draft:
        send_message(chat_id, "⚠️ Время для разделения вышло. Начните ввод расхода заново.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        # Reset user state
        user = db_get_user(user_id)
        if user:
            user['state'] = 'IDLE'
...
def handle_callback(chat_id, user_id, message_id, data, callback_query_id):
    ...
    if cmd == "MENU_TRIPS":
        ...
        edit_message(chat_id, message_id, "🗂 *Архив обязательств*:", reply_markup={"inline_keyboard": keyboard})
        return
    ...
    if cmd == "SWITCH_TRIP":
        ...
        if target_tid in user.get('joined_trips', []):
            ...
            send_message(chat_id, "⚠️ Вы не являетесь участником этой поездки.", reply_markup={"inline_keyboard": [[{"text": "🗂 Архив обязательств", "callback_data": "MENU_TRIPS"}]]})
        return
    ...
    if cmd == "MENU_BALANCE": 
        ...
        keyboard = {"inline_keyboard": [\
            ...
            [{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]\
        ]}\
        ...
        if not simplified_transactions:
            send_message(chat_id, "✅ Балансы уже выровнены, расчетов не требуется!", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
            return
        ...
        send_message(chat_id, "✅ Расчеты инициированы! Всем участникам отправлены личные сообщения с инструкциями.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if cmd == "MENU_ME":
        ...
        send_message(chat_id, report, reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        return
    ...
    if cmd == "TOGGLE":
        ...
        if not active_uids: return edit_message(chat_id, message_id, "❌ Никто не выбран! Выберите хотя бы одного участника.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        ...
        edit_message(chat_id, message_id, f"✅ Сохранено: *{amount:,.0f}* ({CATEGORIES.get(draft.get('category'), draft.get('category'))})\\nРазделено на {len(active_uids)} чел.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})
        notify_others(draft['trip_id'], draft['payer_id'], amount, draft['description'], draft.get('category'), split_map)
        ...
        user['draft_id'] = None
        db_save_user(user_id, user)
        edit_message(chat_id, message_id, "❌ Отменено.", reply_markup={"inline_keyboard": [[{"text": "🔙 Оперативный штаб поездки", "callback_data": "OPEN_DASHBOARD"}]]})

def main():
    print("Splitopus v1.0 (Audit Edition) Started...")
    init_db()
