...
def calculate_balance(trip_id):
    ...
    for exp in trip['expenses']:
        ...
        for uid, share in split.items():
            if str(uid) in balances: balances[str(uid)] -= share
            
    return balances, total_spent
...
def get_my_stats(trip_id, my_uid):
    ...
    for exp in trip['expenses']:
        ...
        if cat == "REPAYMENT":
            ...
            continue # Пропускаем REPAYMENT для обычных трат
...
def send_trip_dashboard(chat_id, user_id, message_id=None):
    ...
    keyboard = {
        "inline_keyboard": [
            ...
            [{"text": "🧾 Все траты", "callback_data": "MENU_ALL_EXPENSES"}],
...
def send_all_expenses_list(chat_id, user_id, message_id=None, page=0):
    ...
    trip = trips[tid]
    expenses = sorted(trip.get('expenses', []), key=lambda x: x['ts'], reverse=True) # Сортируем по дате, новые сверху
    curr = trip.get('currency', 'THB')
    names = {uid: users.get(uid, {}).get('name', 'Unknown') for uid in trip['members']}

    PAGE_SIZE = 10 # Количество трат на страницу
    ...
    for i in range(start_idx, end_idx):
        ...
        # Только для обычных трат, не для возврата долга
        if exp.get('category') != "REPAYMENT":
            ...
            # Для возврата долга показывать "Кто кому вернул"
            repay_from_name = names.get(str(exp['payer_id']), 'Unknown')
            repay_to_id = list(exp['split'].keys())[0] # Получаем ID того, кому вернули
    ...
    if state == "WAITING_ROULETTE_AMOUNT":
        try:
            ...
            new_exp = {
                ...
                "category": "FUN", # Или можно создать новую категорию "ROULETTE"
                "split": split_map,
                "ts": time.time()
            }
            ...
            send_message(chat_id, f"✅ Расход по рулетке *{amount}* добавлен!")
...
def handle_callback(chat_id, user_id, message_id, data):
    ...
    # --- Dashboard Actions ---
    if cmd == "MENU_BALANCE": 
        handle_callback(chat_id, user_id, message_id, "SHOW_MY_BALANCE|OLD") # Re-use legacy logic? No, let's call command
        # Better: Redirect to command logic manually or use same block
        # Actually I removed SHOW_MY_BALANCE recursion in previous fix, but let's be safe:
        # Use command logic directly
        users = load_json(USERS_FILE)
        tid = users[uid_str].get('active_trip_id')
        if not tid: return
        trips = load_json(TRIPS_FILE)
        ...
        curr = trip.get('currency', 'THB')
        rate = trip.get('rate', 0)
        balances, total = calculate_balance(tid)
        names = {uid: users.get(uid, {}).get('name', 'Unknown') for uid in trip['members']}
        
        total_str = f"{total:,.0f} {curr}"
        if rate > 0: total_str += f" (~{total*rate:,.0f} RUB)"
        ...
        for uid, bal in balances.items():
            ...
            emoji = "🟢" if bal >= 0 else "🔴"
            amt = f"{bal:+.0f}"
            if rate > 0: amt += f" ({bal*rate:+.0f} ₽)"
            report += f"{emoji} {name}: {amt}\n"
    ...
    if cmd == "MENU_ME":
        ...
        report = f"👤 *Вы потратили: {stats['total_share']:.0f} {curr}*\n\n"
    ...
    if cmd == "CURR":
        ...
        if curr_code == "CUSTOM":
            ...
            users[uid_str]['state'] = "WAITING_TRIP_RATE"
            save_json(USERS_FILE, users)
            send_message(chat_id, f"💱 Курс 1 {curr_code} к рублю?")
        return
    ...
    if cmd == "SHOW_MY_BALANCE":
        ...
        if tid:
            ...
            if trip:
                curr = trip.get('currency', 'THB')
                rate = trip.get('rate', 0)
                balances, _ = calculate_balance(tid)
